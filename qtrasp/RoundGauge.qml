...
Canvas
{
    ...
    property alias startAngle : d.startAngle;
    property alias endAngle : d.endAngle;
    QtObject
    {
        id : d
        property real range : maxValue - minValue;
        property real startAngle : Math.PI * 1.50;
        property real endAngle : Math.PI * 3.50;
        ...
        Behavior on needleAngleRad {SpringAnimation {spring: 1.5; damping: 1.5;}}
    ...
    onPaint:
    {
        ...
        for (var i = 0; i <= subDivs ; i++)
        {
            ...
            text_model.append({"text" : textVal,
                                  "x" : d.center.x + ((d.radius - ((25 + indicatorWidth) + textFont.pixelSize * 1.5)) * Math.cos(rotAngle)),
                                  "y" : d.center.y + (d.radius - ((25 + indicatorWidth) + textFont.pixelSize * 1.5)) * Math.sin(rotAngle)});
        }
    }
    ...
    Text
    {
        ...
        text : (Math.round(currentValue * 10) / 10).toFixed(2);
        style : Text.Sunken
        styleColor: outerCirclingColor
        color : textColor
    }
